function [fval,x,info]=Knitro_DDFact_heavy(x0,C,s,F,Fsquare,A,b,Gamma, LB)
%% obtain problem data 
A_data=A;
b_data=b;
[m,~] = size(A);
...
%% define nested objective function for Knitro
    function terminate = outfun(x,optimValues,state)
        ...
        if D(end)<1e-15 || min(x) < 1e-15
            info.boundarycount = info.boundarycount + 1;
        end
        sprintf("the s largest eigenvalue: %f, minimum value of x: %f", D(s), min(x));
        terminate = false;
    end
%% calling knitro
...
options = knitro_options('algorithm', 4, 'convex', 1, 'derivcheck', 0, 'outlev', 0 , 'gradopt', 1, ...
