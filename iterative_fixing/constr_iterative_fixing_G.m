...
timelimit = 3600;
filename = strcat('linconstr', num2str(n));
load(filename);
A_data = A;
b_data = b;
MESPInstancefeas = MESP(C,A_data,b_data);
for outs= 4:87
    fprintf("s = %d: \n", outs)
    allfix0=[];
    allfix1=[];
    ...
    while true
        ...
        % updata A,b
        allfix = union(allfix0, allfix1);
        remainid = setdiff(1:Corder, allfix);
        remainid = sort(remainid);
        A = A_data(:, remainid);
        x11 = zeros(Corder,1);
        x11(allfix1)=1;
        b = b_data - A_data*x11;
...
fix=fix63;
filename = strcat('iterative_fixing/constr_fix', num2str(Corder), '_LFcF_gscaling.mat');
save(filename,'fix');
