...
for outs=2:(Corder-1)
    ...
    while true
        ...
        if isempty(info_Linx.fixto0list) && isempty(info_Linx.fixto1list) 
            ...
            fprintf("Before fixing, Linx intgap is %f. \n", ...
                info_Linx.integrality_gap);
            if info_Linx.integrality_gap < 1e-6 
                warning("Intergrality gap too small.\n");
                nearzero_intgap_itr = iteration;
                nearzero_intgap = 1;
            end
            fixto0list=info_Linx.fixto0list;
            fixto1list=info_Linx.fixto1list;
            if iteration == 0
...
fix=fix63;
filename = strcat('iterative_fixing/Linx_fix', num2str(Corder), '_LFcF_gscaling.mat');
save(filename,'fix');
